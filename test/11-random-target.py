...
p.resetDebugVisualizerCamera(cameraDistance=0.75,
                             cameraYaw=135,
                             cameraPitch=-45,
...
read_pos = p.addUserDebugParameter("read pos - slide right".format(i + 1), 0, 1, 0)
read_pos_once = True

start = time.time()

rhis = RandomPointInHalfSphere(0.0, 0.0369, 0.0437,
                               radius=0.2022, height=0.2610, min_dist=0.0477)

dist = DistanceBetweenObjects(bodyA=robot, bodyB=ball.id,
                              linkA=13, linkB=-1)
...
for i in range(frequency * 30):
    ...
    for m in range(len(motors)):
        ...
        motorPos.append(pos)
        p.setJointMotorControl2(robot, motors[m], p.POSITION_CONTROL, targetPosition=pos)
